...
Step 2: Defining the Carrier SignalThe carrier signal is the high-frequency wave used to "carry" the information through space. Here, we define its parameters (frequency = $10\text{ Hz}$, amplitude = $1$) 
and create a time vector spanning from $0$ to $1$ second with $1000$ sample points.
...
Step 3: Defining the Message SignalThe message signal (or modulating signal) represents the actual data or audio we want to transmit. It has a much lower frequency ($2\text{ Hz}$) compared to the carrier wave.
